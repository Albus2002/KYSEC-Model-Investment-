...
def main():
    ...
    tracemalloc.start()
    # 数据集参数
    filepath = 'first_500_rows.csv'
    directory = '/mnt/tmpdata/lvhanglong/data_20240308'  # .parquet文件的路径
    feature_columns = ['Ticker', 'Ret10MinPrice', 'Ret30MinPrice', 
    ...
    # BiAGRU模型参数
    input_size = len(feature_columns)  # 输入特征的数量（排除目标列）
    ...
    # 加载数据
    dataset = StockDataset(filepath, feature_columns, target_column, time_step, split_ratio,directory=directory)
    ...
    # 保存模型
    torch.save(model.state_dict(), 'biagru_stock_model.pth')
    ...
    # 绘制和保存预测结果图像
    predictor.plot_and_save_results(y_test, predictions, file_name='stock_predictions.png')
