...
def grouped_r2_score_multithreaded(output, target, labels, max_workers=32):
    ...
    for i, label in enumerate(labels):
        if (i < l) and np.isnan(output[i]) == False:
...
# test_label = [0 for i in range(74417)]
# val_label = [0 for i in range(74417)]
...
# 实例化模型
model = BiAGRU(input_size = 478, hidden_size = 100,num_layers = 8, dropout = 0.2,output_size = 1)
model.to(device)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % 500 == 0:
            ...
            if val_loss < best_val_loss:
                ...
                best_val_r2 = r2
                # 保存模型参数
                best_model_path = f'../model_0329_BiAGRU/model_best_{epoch}_r2_{best_val_r2}.pth'
