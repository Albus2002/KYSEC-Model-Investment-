...
from model import MLPWithDropout
...
# test_label = [0 for i in range(74420)]
# val_label = [0 for i in range(74420)]
...
train_loader = DataLoader(TensorDataset(torch.tensor(train_X, dtype=torch.float32), 
                                      torch.tensor(train_y, dtype=torch.float32)), batch_size = 50000, shuffle=True,num_workers = 4)

print(f'train dataloader complete:{t - time.time()}')
t = time.time()

val_loader = DataLoader(TensorDataset(torch.tensor(val_X, dtype=torch.float32), 
                                      torch.tensor(val_y, dtype=torch.float32)), batch_size = 50000, shuffle=False,num_workers = 4)

print(f'val dataloader complete:{t - time.time()}')
t = time.time()

test_loader = DataLoader(TensorDataset(torch.tensor(test_X, dtype=torch.float32), 
                                      torch.tensor(test_y, dtype=torch.float32)), batch_size = 50000, shuffle=False,num_workers = 4)

print(f'test dataloader complete:{t - time.time()}')
t = time.time()

# 实例化模型
model = MLPWithDropout(input_size = 478, output_size = 1,dropout_rate = 0.2)
model.to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        best_val_r2 = r2
        # 保存模型参数
        best_model_path = f'../model_0325_dropout/model_best_{epoch}_r2_{best_val_r2}.pth'
